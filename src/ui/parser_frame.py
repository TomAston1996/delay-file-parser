...
class ParserFrame(tk.Frame):
    """Main parser frame for UI

    Args:
        Tk (_type_): Tkinter Frame
    """

    def __init__(self, root: ttk.Notebook, delay_file_reader: DelayFileReader, folder_scanner: FolderScanner) -> None:
        """
        ...
        self.event_filter_label.pack(pady=(35, 5))
    ...
    def run(self) -> None:
        ...
        print(f"[{time_now}][INFO] 📁 Train Delay Directory: {self.search_folder_path.get()}")
        print(f"[{time_now}][INFO] 📁 Destination Directory: {self.destination_folder_path.get()}")
    ...
    def run_excel_bulder(self, excel_builder: ExcelBuilder, progress_tracker: ProgressTracker) -> None:
        ...
        self.after(0, lambda: progress_tracker.update_progress(100, 100))
        self.after(0, lambda: print(f"[{str(datetime.datetime.now())}][INFO] ⚙️ Process complete!"))
        self.after(0, lambda: print(f"[{str(datetime.datetime.now())}][INFO] 🚀 Ready for next action!"))
