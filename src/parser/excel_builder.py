...
class ExcelBuilder:
    ...
    def __format_dataframe(self) -> None:
        ...
        self.excel_dataframe["PlannedTimestamp"] = pd.to_datetime(
            self.excel_dataframe["PlannedTimestamp"], dayfirst=True
        )

        self.excel_dataframe = self.excel_dataframe.sort_values(
            by=["CurrentTiploc", "PlannedTimestamp"], ascending=[True, True]
        )
